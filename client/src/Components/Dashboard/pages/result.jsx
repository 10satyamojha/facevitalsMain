...
const ResultsPage = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        ...
        const response = await axios.get('https://facevital-backend-3.onrender.com/api/healthResults', {
  ...
  const handleDelete = async (resultId) => {
    if (window.confirm('Are you sure you want to delete this result?')) {
      try {
        const token = localStorage.getItem('authToken');
        await axios.delete(`https://facevital-backend-3.onrender.com/api/healthResults/${resultId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
